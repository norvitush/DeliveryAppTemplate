...
#if ADVANCEDINPUTFIELD_TEXTMESHPRO
		[SerializeField] private TMProEmojiAsset emojiAsset;
		private List<InputFieldButton> buttons;

		private void Start()
		{
			if(emojiAsset== null)  emojiAsset = TMP_Settings.defaultSpriteAsset as TMProEmojiAsset;
			print($"emojiAsset: {emojiAsset} TMP_Settings.defaultSpriteAsset {TMP_Settings.defaultSpriteAsset}");
			if(emojiAsset != null)
			{
